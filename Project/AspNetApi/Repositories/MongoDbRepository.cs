...
using AspNetApi.Exceptions;
using AspNetApi.Extensions;
...
namespace AspNetApi.Repositories
{
    public class MongoDbRepository : IRepository
    {
        ...
        public MongoDbRepository()
        {
            ...
            var database = mongoClient.GetDatabase("speedruns");
        ...
        public async Task<SaveData> Load(Guid playerId, NewPlayer newPlayer)
        {
            Player loadingPlayer = new Player(newPlayer);
            Player player = await GetPlayer(playerId);
            var allowed = IsAllowedToEdit(player, loadingPlayer);
            if (!allowed)
                throw new NoPrivilegesException();
        ...
        public async Task<SaveData> Save(Guid playerId, SaveData saveData)
        {
            
            Player player = await GetPlayer(playerId);
            var allowed = CheckIfAllowedToSave(player, saveData);
            if (!allowed)
                throw new NoPrivilegesException();
        ...
        /// <summary>
        /// Returns true if the player is the same as the one with the id that is being saved to.
        /// </summary>
        /// <param name="playerToSave"></param>
        /// <param name="playerThatSaves"></param>
        /// <returns></returns>
        private bool CheckIfAllowedToSave(Player playerToSave, SaveData playerThatSaves)
        {
            if (playerToSave.Name == playerThatSaves.Name && playerThatSaves.Password == playerThatSaves.Password)
                return true;
            return false;
        }

        public async Task<SaveData> DeleteSave(Guid playerId, NewPlayer newPlayer)
        {
            Player player = await GetPlayer(playerId);
            if (player.Name != newPlayer.Name || player.Password != newPlayer.Password)
                throw new NoPrivilegesException();

            SaveData saveData = player.SaveData;
            player.SaveData = null;
            FilterDefinition<Player> filter = Builders<Player>.Filter.Eq(p => p.Id, playerId);
            await collection.ReplaceOneAsync(filter, player);
